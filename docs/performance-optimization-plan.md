...
- Issue #14 was reproduced in `gloomy cave valley - Copy` by opening the map and zooming from `2.0` to `0.1`: visible demand rose from `4` to `782` pages and the application stopped for `8189 ms`. The matching `fastmap_page_db_index_build` took `8177.389 ms` to index `56737` map pieces into `90` page keys on a worker. The main-thread `CheckForExternalMapDbChanges()` poll was blocking on the same `dbLock`, turning background index construction into the visible freeze.
- `CheckForExternalMapDbChanges()` now uses a non-blocking `Monitor.TryEnter(dbLock)` and defers a busy freshness poll to the next one-second interval. Repeating the same automated zoom on the same world reduced the zoom-related application hitch to `355 ms`; the worker index build continued independently and the `782`-page queue drained normally.
- The `355 ms` follow-up exposed a second main-thread DB-lock path: `EnsureVisibleTruePageQueuedOrUploaded()` synchronously called `MightMapDbContainPage()` despite its surrounding frame budget. That probe now stays on the page worker; the main thread only consults the in-memory known-missing set before queueing work. A third identical automated zoom produced no zoom-related hitch above the `100 ms` diagnostic threshold while the worker spent `339.020 ms` building the same `56737`-position index, and all `782` requested pages again drained normally.
