...
The optional `fastmap-terrain-v7` channel negotiates availability, permissions and a server rendering fingerprint. Pregen requests compressed pixel tiles; when enabled, dedicated servers independently prewarm nearby areas and cache all three palette variants. The client submits missing pages through an acknowledged queue; cached pixels stream independently of cold generation, which pushes its results when ready. Detailed grids remain available for optional climate overlays. Timeout, cancellation, and disconnect handling release pending work. Standard Terrain Sampler 1.3.0 uses one background worker; other sampler paths retain a main-thread sampling budget, with rendering and compression on the worker. Earlier measurements below describe the successive development builds explicitly.
...
## PR review regressions, 2026-09-12

- Rendering and compression now use the tile worker even when sampling must remain on the server thread. A blocked-render regression verifies ticks return while the worker is blocked, samples stay on the calling thread, the memory reservation remains held, and no second render is dispatched. Cancellation during this phase sends no late tile.
- Automatic prewarm policy now checks dedicated hosting. Tests with sampling and prewarming explicitly enabled produce no work for an integrated single-player host; dedicated hosting still prepares the target without players or packets. Both opt-out switches suppress work.
- Protocol v7 carries a SHA-256 rendering fingerprint covering effective height/water offsets, snow threshold, world height/sea level, map colours, loaded palette colours and water parameters, sampler version and a renderer revision. Remote disk-cache suffixes use this identity rather than client offsets. Late identity changes replace the layer with fresh immutable cache references, isolating old in-flight work.
- Identity tests cover serialized negotiation, every fingerprint input, stable reuse after restart, changed namespaces, missing or malformed fingerprints and rejection of protocol v6. The network harness passed 2,888 assertions on .NET 8 and 2,889 on .NET 10; the existing worker callback assertions can vary by one with scheduling. Both 1.21.7 and 1.22.7 Release builds passed with zero warnings/errors.
- These review changes were verified in the harness and builds, not a new graphical session. x3200 remains configured with server Pregen disabled. Protocol v7 requires updating both client and server before a subsequent multiplayer Pregen test.

## Handshake layer replacement regression, 2026-09-12

- The first rendering-fingerprint handshake recreated terrain layers by assigning into the live `WorldMapManager.MapLayers` list. Vanilla's concurrent map-worker enumeration could then throw `InvalidOperationException`. Standard replacement, Mapper replacement/insertion, and overlay changes now publish independent lists; existing enumerators retain their original contents and order.
- Retiring a FastMap layer closes worker admission and waits for its active off-thread tick and page/cache/sampler tasks before disposing databases, palettes, or textures. A tick retained by an older list snapshot, or a previously queued task that starts after retirement, cannot access those resources. The previous five-second timeout no longer permits cleanup while a slow worker remains active; retirement may wait for an outstanding I/O or sampling operation to return.
- Regression tests reproduce the original enumerator failure, exercise all list publication operations, and hold two concurrent worker entries during teardown to verify cleanup waits for both and rejects late work. Release builds against 1.21.7 and 1.22.7 passed with zero warnings/errors. The harness passed 2,915 assertions on .NET 8 and 2,916 on .NET 10; the existing callback count varies with scheduling. `git diff --check` passed. No new graphical reconnect test was run, and x3200's disabled Pregen configuration was not changed.

## Persistent server tile cache, 2026-09-12

- Added `PersistTileCache=true`, `TileDiskCacheMegabytes=1024` and `TileCacheRevision=0`. The master Pregen setting remains disabled in packaged defaults. Enabled dedicated servers persist complete compressed palette variants with atomic writes and checksums; disk reads run independently of generation, with reservations included in the RAM budget. Missing/corrupt files regenerate. Disabled persistence leaves existing files intact. Disk quotas include old identities for the same world; speculative writes stop rather than repeatedly evicting current saved targets.
- Both game-version Release builds passed without warnings/errors. Final regression runs passed 2,971 assertions on .NET 8 and 2,974 on .NET 10 (existing worker callback assertions vary with scheduling). New cases cover restart reuse of all palettes without sampling/rendering, persisted-only requests while cold sampling is blocked, permissions, corruption/truncation/invalid lengths/missing files, unavailable storage, disk eviction, cache identity changes, and a 40-page persisted burst within a 1 MiB RAM budget.
- Packaged and validated perf11 archives: 121.6.0 SHA-256 `bcf555d5f01c6cd1400f74e337a5b84cc16035f458861de199b5f80391a68838`; 122.6.0 SHA-256 `0f75dfeb94abc5636b4ced2ec4ccb9e3e06cd6a39be72ed8f881ac20b07c2d70`. Installed the matching 1.22 archive locally and on x3200 through Server Manager MCP, operation `51636b96-fb48-4566-8109-2f0ca48b4a39`. x3200 keeps Pregen and prewarming enabled, radius 12, and now has persistence enabled with the 1,024 MiB disk limit.
- Live first start at 06:47:18 UTC created the new disk namespace. Restart operation `68caa1ca-5b01-4640-aa36-45b789868d55` began at 06:47:55. At 06:48:18, the restarted server restored **36 pages / 9,289,783 bytes** under the same identity. Its first stats showed **zero generated samples, zero RAM-cached pages, 36 disk-cached pages and 589 remaining prewarm targets**, confirming restored pages were skipped without eager RAM loading. Subsequent builds continued from the next uncached coordinates.
- The spawn tile `500_500_4.fmt` was byte-identical before and after restart: 251,099 bytes, SHA-256 `59a3b571c191967d63d62e503961e4201264abbe5c32768284aebcd0e5a7fbb1`. Live restart/index reuse is verified; whole-viewport disk-delivery latency and a new graphical session for this build have not been measured.
